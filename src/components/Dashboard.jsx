...
import { normalizeAllData, getAvailableYears, filterByTime } from '../utils/dataProcessor'
...
export default function Dashboard({ rawData, onRefresh, lastSync }) {
  ...
  const tabs = [{ key: 'all', label: 'All Sheets' }, ...sheets.map(s => ({ key: s, label: s === 'Form Responses 1' ? 'Non DSO' : s }))]
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG }}>
      ...
      <header style={{ background: HEADER_BG }}>
        <div className="max-w-screen-xl mx-auto px-6 pt-5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white leading-none tracking-tight">Stats Analyser</h1>
              <p className="text-indigo-300 text-xs mt-0.5 font-medium">
                {allData.length.toLocaleString()} entries · {sheets.length} sheets
              </p>
            </div>
          </div>
        ...
        <div className="max-w-screen-xl mx-auto px-6 mt-5 flex items-end gap-1 overflow-x-auto">
          {tabs.map(tab => (
            <button
              key={tab.key}
              onClick={() => setActiveSheet(tab.key)}
              className="px-5 py-2.5 text-sm font-semibold rounded-t-xl transition-all whitespace-nowrap flex-shrink-0"
              style={
                activeSheet === tab.key
